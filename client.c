...
int send_packets(int socket_fd, struct sockaddr_in* address, int buf_size, size_t* packet_gap) {
	...
	for (; ; pack_id++) {
		...
		if (sendto(socket_fd, buf, buf_size, 0, (struct sockaddr*)address, sizeof(*address)) != buf_size) {
			perror("sendto()");
		}
		// printf("sent!\n");
...
int main(int argc, char* argv[]) {
	if (argc != 6){
		fprintf(stderr, "usage: %s <server_ip_address> <port> <buf_size> <bitrate>\n", argv[0]);
		exit(1);
	}
	...
	sscanf(argv[5], "%f", &RATE_PRECISION); 
	// target precision
	printf("rate precision: %f\n", RATE_PRECISION);
	...
	send_packets(socket_fd, &address, buf_size, &packet_gap);
